...
int CExperiment::SetNbrofdetectors( TString name )
{
  string buffer;
  int it1;
...
int CExperiment::SetDetectors( TString inputfilename, TString outputfilename , TString Typefilename)
{
  int nbrline = 0;
  int localADCnbr(0), localQDCnbr(0), localRFnbr(0), localSAMPLERnbr(0);
  Int_t detindex_main, chargenumber;
  int it1;
  string buffer_main;
  ...
  Float_t pos_labr, sigma_labr, pos_nai, sigma_nai,theta, cos, sin, tan;
  ...
  while(!PIDliste.eof())
  {
    ...
    if ( inputfile.Index("#") == -1)
    {
      ...
      if(comp == "Q")//---------------------------------- QDC with charge number gates
      {
        ...
        if(vec_theta.size() != 0)
        {
          for(int paris=0; paris < names.size(); paris++)
          {
...
void CExperiment::SetQDCNbr( int nbr)
{
  switch((int)nbr)
  {
    case 2:
...
void CExperiment::SetReferenceDetector(Int_t label)
{
...
{
  TString det_name(0),buffa,buffb,buffc,buffd;
  ...
  // Now I loop on lines
  Calibliste >> det_name >> buffa >> buffb >> buffc >> buffd;
  //cout << "Reading the following from calibration file: " << endl;
  //cout << det_name  << "\t" <<  buffa  << "\t" <<  buffb  << "\t" <<  buffc  << "\t" <<  buffd << endl;;
  while(!Calibliste.eof())
  {
    Calibliste >> det_name >> a >> b >> c >> d;
    TString shortname = det_name(0,5);

    //cout << det_name << "\t" << shortname << "\t" << a << "\t" << b << "\t" << c << "\t" << d << endl;
    //cout << "DetName2nbr["<< det_name << "])=" << DetName2nbr[det_name] << endl;

    if(shortname != "PARIS")GetDetector(DetName2nbr[det_name])->SetCalib(a,b,c,d);
    else
    {
      if(firstparis) {GetDetector(DetName2nbr[det_name])->SetCalib(a,b,c,d);firstparis=false;}
      else {GetDetector(DetName2nbr[det_name])->SetSecondaryCalib(a,b,c,d);firstparis=true;}
    }
...
void CExperiment::SetRUNlistname( TString name )
{
  cout << endl << "---------------------------------------------------------------------" << endl;
