...
Double_t Faster2bitsNRJConverter(nrj_Rawtype enrj, CExperiment *experiment, Int_t detectorindex)
{
  Double_t alea = rand()*1./(RAND_MAX*1.);
  Double_t NRJ = (enrj/(Double_t)experiment->GetDetector((int)detectorindex)->GetMaxchNumber())*(Double_t)experiment->GetDetector((int)detectorindex)->GetNbrChannels()+alea;
  ...
  for(auto &file : experiment->GetDataFileNames())
  {
    ...
    auto loop_and_fill = [&] (TTreeReader &myReader){
      ...
      while(myReader.Next())
      {
        ...
        int spectrumindex = experiment->GetLabel2Detnbrs()[(int)label];
...
std::tuple<int, TGraphErrors*, TF1*, TF1*, TGraph*, Double_t, Double_t , Double_t ,Double_t> OneSpectrumPeakSearchandAnalysis(CExperiment *experiment, Bool_t Calib_BGO, Bool_t Calib_Ge, Bool_t Calib_LaBr, TH1F *spectrum, int sindex, std::vector<Double_t> Ref_nrj, std::vector<Double_t> Ref_nrj_err, std::vector<Double_t> Ref_nrj_intensity)
{
  cout << RESETTEXT << FOREGRN << endl << "Calibrating Detector : " << experiment->GetDetectors().at(sindex)->GetDetectorName() << RESETTEXT << endl;
  // First I define a new spectrum
  TSpectrum *s = new TSpectrum(Ref_nrj.size()*4); // I do not allow to find more than four times the number of reference peaks
  ...
  for(auto &file : experiment->GetDataFileNames())
  {
    ...
    auto loop_and_fill = [&] (TTreeReader &myReader){
      ...
      while(myReader.Next())
      {
        ...
        hitnumber++;threadnbr++;
        if(label > 199) // To make sure I only consider PARIS detectors
        {
          CHit *hit = new CHit(hitnumber+1000*threadnbr);
          hit->SetHit(label, 0, NRJ, NRJ2, false);

          int spectrumindex = experiment->GetLabel2Detnbrs()[(int)label];
          // if(experiment->GetDetectors().at(experiment->GetLabel2Detnbrs()[(int)label])->GetDetectorType() == "PARIS")
          // {
          //   //tie(NRJ2,NRJ) = Rotation((double)experiment->GetDetectors().at(experiment->GetLabel2Detnbrs()[(int)label])->GetRotationAngle(),(double)experiment->GetDetectors().at(experiment->GetLabel2Detnbrs()[(int)label])->GetRotationAngleTan(),NRJ2,NRJ);
          //   //tie(NRJ2,NRJ) = Rotation(0.,0.,NRJ2,NRJ);
          // }

          Double_t PSD = hit->PerformPSD();// PSD = (Long-Short)/Long
          Bool_t isLaBr = experiment->GetDetectors().at(experiment->GetLabel2Detnbrs()[(int)label])->IsPureLaBr3(PSD);
          ...
          if(spectrumindex < PSDSpectra.size())
          {
            ...
            else QDC2Spectra.at(spectrumindex) -> Fill(NRJ2);
          }
    ...
    auto loop_and_fill = [&] (TTreeReader &myReader){
      ...
      while(myReader.Next())
      {
        ...
        if(label > 199) // To make sure I only consider PARIS detectors
        {
          ...
          Double_t PSD = hit->PerformPSD();// PSD = (Long-Short)/Long
          Bool_t isLaBr = experiment->GetDetectors().at(experiment->GetLabel2Detnbrs()[(int)label])->IsPureLaBr3(PSD);
          Bool_t isBeyondLaBrandNaI = experiment->GetDetectors().at(experiment->GetLabel2Detnbrs()[(int)label])->IsBeyondLaBr3andNaI(PSD);
          ...
          if(spectrumindex < PSDSpectra.size())
          {
            ...
            else if(!isBeyondLaBrandNaI)QDC2Spectra.at(spectrumindex) -> Fill(NRJ2);
          }
          if(spectrumindex < PSDMatrix.size())
          {
            //cout << "Hit # " << hitnumber+1000*threadnbr << "; Filling Matrix " << spectrumindex << endl;
...
std::tuple<std::vector<Double_t>,std::vector<Double_t>> PSDSpectrumAnalyzer(TH1F *spectrum)
{
  ...
  std::vector<Double_t> vec_sigma;Double_t sigma(0.3);
  TSpectrum *s = new TSpectrum(6);
  cout << "Searching for peaks" << endl;
  int nfound = s->Search(spectrum,0.2,"",0.2);
  ...
  if(nfound >= 2)
  {
    ...
    // Je ne fit que le premier qui ne doit pas être loin de zero
    Double_t xp = xpeak.at(0);
...
int NRJCalibrator(CExperiment *experiment,TChain *chained_oak, DynamicProgress<ProgressBar> &BAR, int localchainnbr, mutex *stop)
{
  ...
  nrj_type nrj,nrj2,nrj3,nrj4;
  label_type index;
  Bool_t pileup;
  tm_type tm;

  // Declaration of old tree variables
  nrj_Rawtype  enrj,enrj2,enrj3,enrj4;
  label_type index1;
  Bool_t pileup1;
  tm_Rawtype tm1;

  chained_oak->SetBranchAddress ("label", &index1);
  ...
  chained_oak->SetBranchAddress ("time", &tm1);
  chained_oak->SetBranchAddress ("pileup",&pileup1);
  ...
  if(experiment->GetisQDC2()) oak->Branch ("nrj2", &nrj2);
  if(experiment->GetisQDC3()) oak->Branch ("nrj3", &nrj3);
  if(experiment->GetisQDC4()) oak->Branch ("nrj4", &nrj4);
  ...
  // I only have one charge per detector
  if(!experiment->GetisQDC2())
  {
    for (auto hitI : ROOT::TSeqUL(localchainentries))
    {
      if(hitI%percent==0) // Printing every X percent
      {
        BAR[localchainnbr].set_progress((int)((double)hitI/(double)chainentries*100.));//printProgress((double)hitI/(double)chainentries);
      }

      chained_oak->GetEntry(hitI);

      int detindex = experiment->GetLabel2Detnbrs()[(int)index1];
      Double_t NRJ = enrj;//Faster2bitsNRJConverter(enrj1, experiment, detindex); // To convert to a sort of integer (it should remove the "non-linearity")
      ...
      //cout << "enrj1 = " << enrj1 << "\t" <<"NRJ = " << NRJ << "\t" << "enrj = " << enrj << endl;
      if(enrj > 10.) oak->Fill();
    }
  }

  // Some detectors have two charges
  if(experiment->GetisQDC2() && !experiment->GetisQDC3())
  {
    for(auto hitI : ROOT::TSeqUL(localchainentries))
    {
      if(hitI%percent==0) // Printing every X percent
      {
        ...
        BAR[localchainnbr].set_option(option::PostfixText{
                                      std::to_string((int)hitI) + "/" + std::to_string((int)localchainentries)
                                      });
      ...
      //stop.lock();
      int testentry = chained_oak->GetEntry(hitI);
      //cout << "ZEntry  size " << testentry << endl;
      if(testentry > 0)
      {
        ...
        if(index1 > 199)
        {
          ...
          hit->SetHit(index, 0, enrj, enrj2, false);
          Double_t PSD = hit->PerformPSD();// PSD = (Long-Short)/Long
          Bool_t isLaBr = experiment->GetDetector(detindex)->IsPureLaBr3(PSD);
          Bool_t isBeyondLaBrandNaI = experiment->GetDetector(detindex)->IsBeyondLaBr3andNaI(PSD);
